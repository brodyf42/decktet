module Decktet
  class Card
    ...
    def ace?()    rank == :ace          end
    def number?() (2..9).include?(rank) end
    def pawn?()   rank == :pawn         end
    def court?()  rank == :court        end
    def crown?()  rank == :crown        end

    def location?()     has_type?(:location)     end
    def personality?()  has_type?(:personality)  end
    def event?()        has_type?(:event)        end
